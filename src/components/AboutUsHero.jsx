...
const AboutUsHero = () => {
  ...
  return (
    <div ref={ref} className="relative w-full h-screen max-sm:h-[50vh]">
      ...
      <div className="flex h-full w-full">
        ...
        >
          ...
          <motion.div 
            className="absolute bottom-0 left-0 p-3 sm:p-10"
            initial={{ opacity: 0, y: 50 }}
            animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
            transition={{ duration: 0.8, delay: 0.3, ease: "easeOut" }}
          >
            <h1 className="text-white text-2xl sm:text-8xl font-bold tracking-wider">
              ABOUT US
            </h1>
          </motion.div>
        </motion.div>
        ...
        <motion.div 
          className="w-1/3 h-full bg-gradient-to-r from-[#DBB965] from-0% to-[#564827] to-78% flex items-center justify-center p-3 sm:p-10"
          initial={{ opacity: 0, x: 100 }}
          animate={isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: 100 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
        >
